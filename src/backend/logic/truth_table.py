...
def _evaluate_all(circuit: Any, input_values: dict[int, int]) -> dict[int, int]:
    ...
    def resolve(node_id: int) -> int:
        ...
        if node_type == "IN":
            ...
            # OUT is a passthrough node: it forwards the value from its single input.
            ...
            # Input pins are positional, so we require a complete set 0..required-1.
...
def get_truth_table(circuit: Any) -> dict[str, Any]:
    ...
    circuit.accept(roles)
    inputs = sorted(roles.input_ids)
    outputs = sorted(roles.output_ids)
    if not inputs or not outputs:
        return {"inputs": inputs, "outputs": outputs, "rows": []}

    rows: list[dict[str, int]] = []
    # Full truth table is generated by iterating over all 2^N input combinations.
...
def simplify(circuit: Any, input_values: dict[int, int]) -> Any:
    ...
    for node_id, value in fixed.items():
        node = new_circuit.get_node(node_id)
        if node and _normalize_type(node["type"]) == "IN":
            new_circuit.set_node_type(
                node_id, "CONST_1" if value else "CONST_0"
            )

    # After replacing fixed inputs with constants, prune everything
    # that no longer contributes to any OUT node.
